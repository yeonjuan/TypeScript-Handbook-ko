...
`padLeft`의 원본에서 좋은 점은 그냥 원시 값 (primitives)을 전달할 수 있다는 것입니다.
즉 사용법이 간단하고 간결합니다.
...
세로 막대 (`|`)로 각 타입을 분리하여 사용합니다. 그래서 `숫자 | 문자열 | 불리언`은 값의 타입이 `숫자`, `문자열` 혹은 `불리언`이 될 수 있음을 나타냅니다.
...
유니언 타입은 여기서 약간 까다로울 수 있으나, 익숙해지는데 약간의 직관 (a bit of intuition)만 필요할 뿐입니다.
