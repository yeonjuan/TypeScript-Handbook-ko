...
열거형을 사용하면 이름이 있는 상수들의 집합을 정의할 수 있습니다.
열거형을 사용하면 명확히 구분되는 사례들의 집합을 만들 수 있으며, 문서를 좀 더 명확하게 작성할 수 있습니다.
TypeScript에서는 숫자와 문자를 이용하여 열거형 데이터를 만들 수 있습니다.
...
위 코드에서 `Up`을 `1` 으로 초기화하여 숫자 열거형을 선언했습니다.
이러면 뒤따르는 멤버들은 자동-증가한 값을 갖습니다.
...
자동-증가하는 기능은 멤버 값 자체에는 신경 쓰지 않지만, 각 값이 같은 열거형의 다른 값과 구별돼야 하는 경우에는 유용합니다.

열거형을 사용하는 것은 간단합니다: 그냥 열거형 그 자체에서 프로퍼티로 모든 멤버에 접근하며, 열거형의 이름을 사용하여 타입을 선언합니다.
...
문자열 열거형에서 각 멤버들은 문자 리터럴 또는 다른 문자열 열거형의 멤버로 상수 초기화 해야 합니다.
...
다시 말해서 만약 당신이 숫자 열거형을 이용해서 디버깅하고 있고 그 값을 읽어야 한다면, 종종 그 값이 불투명한 경우가 있습니다 - 숫자만으로는 이것이 어떤 의미인지 유의미한 정보를 제공해주지 않기 때문입니다. ([역 매핑](#역-매핑-Reverse-mappings) 을 이용하면 도움이 될지라도 말입니다), 반면 문자열 열거형을 이용하면 당신이 코드를 실행할 때, 열거형 멤버에 지정된 이름과는 무관하게 유의미하고 읽기 좋은 값을 이용하여 실행할 수 있도록 도와줍니다.

## 이종 열거형 (Heterogeneous enums)

기술적으로 열거형은 숫자와 문자를 섞어서 사용할 수 있지만 굳이 그렇게 사용해야 할 이유가 없습니다.
...
* 초기화 값이 없으며 앞의 열거형 멤버는 *숫자* 상수입니다.
  이런 경우 앞에 나온 상수 값에 1씩 증가한 값을 상수로 갖습니다.
...
계산되지 않은 상수 열거형에는 특수한 부분 집합인 리터럴 열거형 멤버가 있습니다.
리터럴 열거형 멤버는 초기화 값이 존재하지 않거나 초기화된 상수 열거형 멤버입니다.
...
그러나 만약 이 조건이 false 라면, `x` 는 반드시 `E.Foo` 이기 때문에, x가 `E.Bar` 가 아닌지 묻는 조건과 비교하는 것은 반드시 true를 반환하기 때문에 말이 되지 않습니다.
